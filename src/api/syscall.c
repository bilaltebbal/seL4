...
extern uint64_t timerTickCounter;
extern tcb_t *SleeperQueue;
...
handleSleep(void)
{
    ...
    if(sleeplen <= 0 || sleeplen > 100000){
        ...
        NODE_STATE(ksCurThread)->sleepcount = sleeplen;
        NODE_STATE(ksCurThread)->tcbSleepNext = SleeperQueue;
        SleeperQueue = NODE_STATE(ksCurThread);
